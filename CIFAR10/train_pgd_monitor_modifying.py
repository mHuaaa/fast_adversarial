...
def main():
    ...
    logger.info('Epoch \t Seconds \t LR \t \t avg_Kt \t Alpha \t Train Loss \t Train Acc')
    ...
    for epoch in range(args.epochs):
        ...
        candidate_parses, candidate_probs, next_attack_parses = chooseAttackParsesV2(attack_parses, eval_loss, eval_acc,
                                                            alpha_interval=args.alpha_interval, mode=args.mode)

        candidate_probs_cumsum = np.cumsum(candidate_probs)
        print("candidate_probs_cumsum", candidate_probs_cumsum)
        total_loss = 0
        total_n = 0
        ...
        for i, (X, y) in enumerate(train_loader):
            # select Alphat and Kt by probs

            choose_index = np.searchsorted(candidate_probs_cumsum, np.random.rand())
            Kt, Alphat = candidate_parses[choose_index]
            # print("random choose:", choose_index, candidate_parses)
            ...
            sum_Kt += Kt*y.size(0)
            # print(sum_Kt, y.size(0), train_n)
        epoch_time = time.time()
        lr = scheduler.get_lr()[0]
        logger.info('%d \t %.1f \t \t %.4f \t %.2f \t %d \t %.4f \t %.4f',
                    epoch, epoch_time - start_epoch_time, lr, sum_Kt/train_n, Alphat, train_loss/train_n, train_acc/train_n)
