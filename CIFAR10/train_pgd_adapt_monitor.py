...
def get_args():
    ...
    parser.add_argument('--adapt-mode', default=0, type=float)
    parser.add_argument('--adapt-alpha-min', default=0, type=float)
...
def adaptCompute(Acct, args):
    if args.adapt_mode == 0:
        Alphat = 7/(1+np.exp(10*(Acct-0.5)))+1
        Kt = 10*Acct/Alphat
    elif args.adapt_mode == 1:
        Alphat = 7/(1+np.exp(18*(Acct-0.5)))+1
        Kt = 10*Acct/Alphat
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        last_acc = train_acc/train_n
        logger.info('%d \t %.1f \t \t %.4f \t %d \t %d \t %.4f \t %.4f',
            epoch, epoch_time - start_epoch_time, lr, Kt, Alphat, train_loss/train_n, last_acc)
