...
from utils import (upper_limit, lower_limit, std, clamp, get_loaders,
    evaluate_pgdV3, evaluate_standard)
...
def main():
    ...
    for i in range(start, end+1, step):
        ...
        loss1, acc1, output1, loss2, acc2, output2, y = evaluate_pgdV3(data_loader, model_test, args.restarts)
        # test_loss, test_acc = evaluate_standard(data_loader, model_test)

        # logger.info('%d \t %d \t %d \t %.4f \t %.4f', i, attack_iters, alpha, pgd_loss, pgd_acc)
        logger.info('%d \t %.4f \t %.4f \t %.4f \t %.4f', i, loss1, acc1, loss2, acc2,)
        data = [output1.cpu().numpy(), output2.cpu().numpy(), y.cpu().numpy()]
        with open(os.path.join(args.model_dir, "data_test.pkl"), "wb") as f:
            pkl.dump(data, f)

        break

        # print(output1.cpu().numpy())
        # print(output2.numpy())
        # print(y.numpy())
