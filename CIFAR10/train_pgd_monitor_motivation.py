...
def get_args():
    ...
    parser.add_argument('--in-model-path', type=str, help='input model path')
...
def main():
    ...
    if args.in_model_path:
        model.load_state_dict(torch.load(args.in_model_path))
    model.train()
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if epoch < 5 and batch_cnt % 50 == 0:
                print("save model: {}; batch_cnt: {}".format(model_cnt, batch_cnt))
                torch.save(model.state_dict(), os.path.join(args.out_dir, 'model_'+str(model_cnt)+ '.pth'))
                model_cnt += 1
